 ...
 * Mirrors the official DSH client-bundle preset and dsh-workspace-explorer's
 * verified tsdown.config.ts: externals resolve through the loader module
 * table; CSS modules compile inline via lightningcss and inject
 * <style data-plugin-css> tags; everything else inlines.
 */
...
const nodeLib: UserConfig = {
  ...
  entry: [
    'src/index.ts',
    'src/host/process-inspector.ts',
    'src/host/redaction.ts',
    'src/host/ownership.ts',
    'src/host/config.ts',
    'src/host/ledger.ts',
    'src/host/lifecycle.ts',
    'src/host/service-registry.ts',
  ],
  outDir: 'lib',
  format: ['esm'],
  platform: 'node',
