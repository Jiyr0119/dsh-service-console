...
async function scanRaw(commandRunner = runCommand) {
	const partialWarnings = [];
	const tcp = await commandRunner("lsof -nP -iTCP -sTCP:LISTEN", 5e3);
	...
	if (pids.length > 0) {
		const psRes = await commandRunner("ps -o pid=,ppid=,lstart=,command= -p " + pidList, 5e3);
		const psRows = parsePsRows(psRes.stdout);
		if (psRes.code !== 0 && psRes.code !== null) partialWarnings.push("ps detail partial");
		const pgidRes = await commandRunner("ps -o pid=,pgid= -p " + pidList, 5e3);
		...
		const cwdRes = await commandRunner("lsof -a -p " + pidList + " -d cwd", 5e3);
