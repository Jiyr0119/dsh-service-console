		...
		const routes = [
			{
				...
				handler: async () => ({
					ok: true,
					data: await buildSnapshot(shell, ledger.list, workspaces())
				})
			},
			{
				path: "/dsh-sc/api/services/detail",
				handler: async (args) => {
					const svc = (await buildSnapshot(shell, ledger.list, workspaces())).services.find((x) => x.id === String(args.serviceId ?? ""));
					if (!svc) return {
						ok: false,
						error: {
							code: "TARGET_GONE",
